...
export default function Tool() {
  ...
  const handleEnhanceAll = async () => {
    ...
    const enhancedResults: ImageItem[] = await Promise.all(
      images.map(async (img) => {
        ...
        try {
          ...
          return {
            ...
            isEnhanced: true,
            enhancedUrl,
            suggestions: [...img.suggestions, enhancedSuggestion],
          } as ImageItem;
        } catch (err) {
          return { ...img, isEnhancing: false, isEnhanced: false } as ImageItem;
        }
      })
    );
    setImages(enhancedResults);
    // Persist previews for landing page
    try {
      const previews = enhancedResults.map(i => ({ url: i.enhancedUrl || i.originalUrl, enhanced: i.isEnhanced, validated: i.isValidated }));
      localStorage.setItem('recentPreviews', JSON.stringify(previews));
    } catch {}
  ...
  const handleValidateAll = async () => {
    ...
    const validatedResults: ImageItem[] = await Promise.all(
      images.map(async (img) => {
        const formData = new FormData();
        formData.append('image', img.file);
        try {
          ...
          const validationSuggestion: Suggestion = {
          ...
          return {
            ...
            isValidating: false,
            isValidated: true,
            suggestions: [...img.suggestions, validationSuggestion],
          } as ImageItem;
        } catch (err) {
          return { ...img, isValidating: false, isValidated: false } as ImageItem;
        }
      })
    );
    ...
    // Persist previews for landing page
    try {
      const previews = validatedResults.map(i => ({ url: i.enhancedUrl || i.originalUrl, enhanced: i.isEnhanced, validated: i.isValidated }));
      localStorage.setItem('recentPreviews', JSON.stringify(previews));
    } catch {}
